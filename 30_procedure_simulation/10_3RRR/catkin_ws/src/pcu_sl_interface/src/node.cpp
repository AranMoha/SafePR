...
int publishData(int ii, TaskHandle taskHandle, ros::Publisher& pub_ni_kmsmini40) {
    ...
    // ros::Rate rate(1000); // Veröffentlichungsrate 1 Hz
}
...
protected:
    ...
    // ros::Subscriber sub_realsense_imu;
    // ros::Subscriber sub_realsense_ang;
    // ros::Subscriber sub_realsense_acc;
        ...
        was_acquired_(false) // Start ohne Verbindung zu Client
{
        ...
        // sub_realsense_imu = nh_.subscribe("/camera/imu", 1, &SLNode::sub_realsense_imu_cb, this);
        // sub_realsense_ang = nh_.subscribe("/camera/gyro/sample", 1, &SLNode::sub_realsense_ang_cb, this);
        // sub_realsense_acc = nh_.subscribe("/camera/accel/sample", 1, &SLNode::sub_realsense_acc_cb, this);
    ...
    void run(){
        ...
        DAQmxCreateAIVoltageChan(taskHandle, "Dev2/ai0:5", "", DAQmx_Val_Cfg_Default, -10.0, 10.0, DAQmx_Val_Volts, NULL);
...
private:
    ...
    void sub_realsense_ang_cb(const sensor_msgs::Imu::ConstPtr& msg)
    {
        // Callback-Funktion für Subscriber von camera imu
        // Wird aufgerufen, wenn neue ROS-Message dafür ankommt
        pthread_mutex_lock(&mut_in_); // Sperre Zugriff auf sl_in_buffer_
        sl_in_buffer_.realsense_gyro[0] = msg->angular_velocity.x;// Daten von ROS-Message in Buffer schreibe
        sl_in_buffer_.realsense_gyro[1] = msg->angular_velocity.y;
        sl_in_buffer_.realsense_gyro[2] = msg->angular_velocity.z;
        pthread_mutex_unlock(&mut_in_); // Freigabe von sl_in_buffer_
    }

    void sub_realsense_acc_cb(const sensor_msgs::Imu::ConstPtr& msg)
    {
        // Callback-Funktion für Subscriber von camera imu
        // Wird aufgerufen, wenn neue ROS-Message dafür ankommt
        pthread_mutex_lock(&mut_in_); // Sperre Zugriff auf sl_in_buffer_
        sl_in_buffer_.realsense_acc[0] = msg->linear_acceleration.x;
        sl_in_buffer_.realsense_acc[1] = msg->linear_acceleration.y;
        sl_in_buffer_.realsense_acc[2] = msg->linear_acceleration.z;
        pthread_mutex_unlock(&mut_in_); // Freigabe von sl_in_buffer_
    }
    void sub_realsense_imu_cb(const sensor_msgs::Imu::ConstPtr& msg)
    {
        // Callback-Funktion für Subscriber von camera imu
        // Wird aufgerufen, wenn neue ROS-Message dafür ankommt
        pthread_mutex_lock(&mut_in_); // Sperre Zugriff auf sl_in_buffer_
        sl_in_buffer_.realsense_imu[0] = msg->linear_acceleration.x;
        sl_in_buffer_.realsense_imu[1] = msg->linear_acceleration.y;
        sl_in_buffer_.realsense_imu[2] = msg->linear_acceleration.z;
        sl_in_buffer_.realsense_imu[3] = msg->angular_velocity.x;
        sl_in_buffer_.realsense_imu[4] = msg->angular_velocity.y;
        sl_in_buffer_.realsense_imu[5] = msg->angular_velocity.z;
        pthread_mutex_unlock(&mut_in_); // Freigabe von sl_in_buffer_
    }
