function [predLoc, predForce, x_P,tauext_est_mean] = LocIsol_particleFilter(trigger, Tauext, predBody, q, dq, x, dx, Jinv_hqx_ind, J_hredxqa, n_particle, param)
    ...
    for k = 1:n_particle
        ...
        x_P_update(k,1) = (x_P_est(k,1) + sqrt(param.x_N_3D(1))*randn);
        x_P_update(k,2) = (x_P_est(k,2) + sqrt(param.x_N_3D(2))*randn);
        x_P_update(k,3) = (x_P_est(k,3) + sqrt(param.x_N_3D(3))*randn);
        ...
        P_w(k) = P_wtmp * exp(-0.5*(z-z_update(k,:)')'*param.x_CovR_Inv_3D*(z-z_update(k,:)'));
    end
