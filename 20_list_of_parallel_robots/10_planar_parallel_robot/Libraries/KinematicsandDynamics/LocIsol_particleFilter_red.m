function [predLoc, predForce, x_P, tauext_est_mean] = LocIsol_particleFilter_red(trigger, Tauext, predBody, q, dq, x, dx, Jinv_hqx_ind, J_hredxqa, ...
    n_particle, l_all, r_BasisPla_MobPla, l_all_id, r_fixPlat_j, r_mobPlat_j, psi_j, x_N_2D, x_CovR_Inv_2D)
    ...
    for k = 1:n_particle
        x_P_update(k,1) = (x_P_est(k,1) + sqrt(x_N_2D(1))*randn);
        x_P_update(k,2) = (x_P_est(k,2) + sqrt(x_N_2D(2))*randn);
        ...
        P_w(k) = P_wtmp * exp(-0.5*(z-z_update(k,:)')'*x_CovR_Inv_2D*(z-z_update(k,:)'));
    end
